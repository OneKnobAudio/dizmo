use crate::ui::{ACCENT, CARD_BG, FIELD_BG, INDICATOR, KNOB_BORDER, TEXT_DIM};
...
/// Dragging horizontally sets the pan value; double clicking resets it to center. When `enabled`
/// is `false` (MULTI mode) the knob is drawn dimmed and does not respond to input.
pub fn show_knob(
    ui: &mut Ui,
    setter: &ParamSetter,
    pan: &FloatParam,
    ...
    painter.line_segment(
        ...
        Stroke::new(2.5, if enabled { INDICATOR } else { KNOB_BORDER }),
    );
